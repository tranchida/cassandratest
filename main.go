...
func main() {
	...
	shutdownFunc, err := setupOTelSDK(context.Background())
	if err != nil {
		fmt.Println("Error setting up OpenTelemetry SDK:", err)
		return
	}
	defer func() {
	...
	// Set up Cassandra database connection
	repo, err := factory.NewRepository("cassandra")
	if err != nil {
		panic(err)
	}
	defer repo.Close()
